<?php

ob_start();

require_once("include/initialize.php");

global $database;

$fabric_id = 0;
$limit = PAGE_LIMIT;

if (isset($_GET['id']) && $_GET['id'] > 0){

    $fabric_id = $_GET['id'];

    $sql = "SELECT f.id,f.fullname, LOWER(t.fullname) type_name
            FROM tbl_fabric f
            INNER JOIN tbl_type t ON t.id = f.type_id
            WHERE f.isactive = 1 AND f.id = '$fabric_id'";

    $result = $database->query($sql);
    $row = $database->fetch_array($result);
    $fabric_name = $row["fullname"];
    $type_name = $row["type_name"];

    $sql = "SELECT id article_id,fullname,isactive,image_name FROM tbl_article WHERE isactive = 1 AND fabric_id = '$fabric_id' ORDER BY id LIMIT 0,$limit";
    $result = $database->query($sql);

    $sql1 = "SELECT COUNT(1) num_rows FROM tbl_article WHERE isactive = 1 AND fabric_id = '$fabric_id'";
    $result1 = $database->query($sql1);
    $row1 = $database->fetch_array($result1);

    $postCount = $row1['num_rows'];


}else{
    redirect_to("index.php");
}

?>

<!DOCTYPE html>
<html lang="en">

    <head>
        <meta http-equiv="content-type" content="text/php; charset=utf-8">
        <meta name="author" content="TechyDevs">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <meta http-equiv="X-UA-Compatible" content="ie=edge">

        <title><? echo $fabric_name ?> | Everything Bespoke</title>

        <!-- Favicon -->
        <link rel="icon" sizes="16x16" href="images/favicon.png">

        <!-- Google Fonts -->
        <link href="https://fonts.googleapis.com/css?family=Poppins:100,200,300,400,500,600,700,800,900&amp;display=swap"
              rel="stylesheet">

        <!-- inject:css -->
        <link rel="stylesheet" href="css/bootstrap.min.css">
        <link rel="stylesheet" href="css/font-awesome.min.css">
        <link rel="stylesheet" href="css/line-awesome.css">
        <link rel="stylesheet" href="css/animate.min.css">
        <!--<link rel="stylesheet" href="css/owl.carousel.min.css">-->
        <link rel="stylesheet" href="css/owl.theme.default.min.css">
        <link rel="stylesheet" href="css/jquery.countdown.css">
        <link rel="stylesheet" href="css/daterangepicker.css">
        <link rel="stylesheet" href="css/magnific-popup.css">
        <link rel="stylesheet" href="css/jquery.fancybox.css">
        <link rel="stylesheet" href="css/progresscircle.css">
        <link rel="stylesheet" href="css/style.css">

        <link rel="stylesheet" href="css/lightbox/lightbox.min.css">
        <link rel="stylesheet" href="css/fabric.css">
        <!-- end inject -->

        <style>
            #loadBtn {
                background-color: #000000;
                padding: 8px 17px;
                color: #fff;
                border-radius: 5px;
                font-size: 17px;
            }
            .loadmore {
                text-align: center;
                margin-top: 10px;
            }
        </style>

    </head>

    <body>

    <!-- start loading -->
    <div id="loading">
        <div id="loading-center">
            <div id="loading-center-absolute">
                <div class="object" id="object_one"></div>
                <div class="object" id="object_two"></div>
                <div class="object" id="object_three"></div>
            </div>
        </div>
    </div>
    <!-- end loading -->

    <?php include "include/header.php" ?>

    <!--================================
            START BREADCRUMB AREA
    =================================-->
    <section class="breadcrumb-area section-about-breadcrumb">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <div class="breadcrumb-content">
                        <div class="breadcrumb__titles">
                            <h2 class="breadcrumb__title"><? echo $fabric_name ?></h2>
                        </div>
                        <ul class="breadcrumb__list">
                            <li class="breadcrumb__list-active"><a href="index.php">Home</a></li>
                            <li><? echo $fabric_name ?></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--================================
            END BREADCRUMB AREA
    =================================-->

    <section class="section portfolio-area portfolio-area3 portfolio-area5 section-padding">
        <div class="container">
            <div class="row">
                <div class="col-lg-12 portfolio-wrap">
                    <div class="row portfolio-list">

                            <? while ($row = $database->fetch_assoc($result)){ ?>
                                <? $image_name = ARTICLE_URL.$type_name.'/'.$row["image_name"]; ?>
                                <div class="single-portfolio-item col-lg-3 col-sm-6 all">
                                    <div class="portfolio-item" style='background-image: url("<? echo $image_name ?>");'>
                                        <a href="<? echo $image_name ?>" data-lightbox="roadtrip" data-lightbox="image-1" data-title="Article # <?php echo $row["fullname"]; ?>" class="portfolio-desc">
                                            <img src="<? echo $image_name ?>" alt="<?php echo $row["fullname"]; ?>" />
                                        </a>
                                    </div>
                                    <div class="article-name">
                                        <span>Article # <?php echo $row["fullname"]; ?></span>
                                    </div>
                                </div>

                            <? } ?>

                    </div>

                    <div class="loadmore">
                        <input type="button" id="loadBtn" value="Load More">
                        <input type="hidden" id="row" value="0">
                        <input type="hidden" id="postCount" value="<?php echo $postCount; ?>">
                    </div>

                </div>
            </div>
        </div>
    </section>

    <div class="section-block"></div>

    <?php include "include/footer.php" ?>

    <!-- theme js files -->
    <script src="js/jquery-1.10.2.min.js"></script>
    <script src="js/popper.min.js"></script>
    <script src="js/bootstrap.min.js"></script>
    <!--<script src="js/owl.carousel.min.js"></script>-->
    <script src="js/magnific-popup.min.js"></script>
    <script src="js/waypoint.min.js"></script>
    <script src="js/jquery.counterup.min.js"></script>
    <script src="js/isotope.js"></script>
    <script src="js/wow.js"></script>
    <script src="js/jquery.fancybox.min.js"></script>
    <script src="js/moment.min.js"></script>
    <script src="js/progresscircle.js"></script>
    <script src="js/smooth-scrolling.js"></script>
    <script src="js/daterangepicker.js"></script>
    <script src="js/tilt.jquery.min.js"></script>
    <script src="js/main.js"></script>

    <script src="js/lightbox/lightbox.min.js"></script>

    <!--https://www.cluemediator.com/load-more-data-from-the-database-using-ajax-jquery-in-php-with-mysql-->

    <script>

        let limit = '<? echo $limit ?>'
        let fabric_id = '<? echo $fabric_id ?>'
        let type_name = '<? echo $type_name ?>'

        $(document).ready(function () {

            if ($('#postCount').val() <= 20){
                $('#loadBtn').css("display", "none");
            }

            $(document).on('click', '#loadBtn', function () {

                let row = Number($('#row').val());
                let count = Number($('#postCount').val());
                row = Number(row) + Number(limit);

                console.log("row", row)


                $('#row').val(row);
                $("#loadBtn").val('Loading...');

                $.ajax({
                    type: 'POST',
                    url: 'include/ajax/action_loadmore.php',
                    data: {
                        'action': "articles",
                        'fabric_id' : fabric_id,
                        'type_name' : type_name,
                        'row':row
                    },
                    success: function (data) {

                        //console.log(data)
                        let rowCount = Number(row) + Number(limit);
                        let rowRemain = Number(count) - Number(rowCount);
                        console.log("rowRemain", rowRemain)

                        $('.portfolio-list').append(data);

                        console.log("rowCount",rowCount)
                        //$("#loadBtn").val('Load More');

                        if (rowCount >= count) {
                            $('#loadBtn').css("display", "none");
                        } else {
                            $("#loadBtn").val('Load More');
                        }
                    }
                });
            });
        });
    </script>

    </body>

</html>
