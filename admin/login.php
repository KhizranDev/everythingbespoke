<?php ?>

<!DOCTYPE html>
<html lang="en">
<head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>Everything Bespoke Admin</title>
    <!-- plugins:css -->
    <link rel="stylesheet" href="assets/vendors/mdi/css/materialdesignicons.min.css">
    <link rel="stylesheet" href="assets/vendors/css/vendor.bundle.base.css">
    <!-- endinject -->
    <!-- Plugin css for this page -->
    <!-- End plugin css for this page -->
    <!-- inject:css -->
    <!-- endinject -->
    <!-- Layout styles -->
    <link rel="stylesheet" href="assets/css/style.css">
    <!-- End layout styles -->
    <link rel="shortcut icon" href="assets/images/favicon.png" />
</head>
<body>
<div class="container-scroller">
    <div class="container-fluid page-body-wrapper full-page-wrapper">
        <div class="row w-100 m-0">
            <div class="content-wrapper full-page-wrapper d-flex align-items-center auth login-bg">
                <div class="card col-lg-4 mx-auto">
                    <div class="card-body px-5 py-5">
                        <h3 class="card-title text-left mb-3">Login</h3>

                        <form id="frmLogin">
                            <div class="form-group">
                                <label>Username or email *</label>
                                <input type="text" class="form-control p_input" id="txtUserId" name="txtUserId">
                            </div>
                            <div class="form-group">
                                <label>Password *</label>
                                <input type="password" class="form-control p_input" id="txtPassword" name="txtPassword">
                            </div>
                            <div class="form-group d-flex align-items-center justify-content-between">
                                <div class="form-check">
                                    <label class="form-check-label">
                                        <input type="checkbox" class="form-check-input"> Remember me </label>
                                </div>
                                <a href="#" class="forgot-pass" style="display: none;">Forgot password</a>
                            </div>
                            <div class="text-center">
                                <button type="button" class="btn btn-primary btn-block enter-btn" id="btnLogin">Login</button>
                            </div>
                            <p class="sign-up">Don't have an Account?<a href="#"> Sign Up</a></p>
                        </form>

                    </div>
                </div>
            </div>
            <!-- content-wrapper ends -->
        </div>
        <!-- row ends -->
    </div>
    <!-- page-body-wrapper ends -->
</div>
<!-- container-scroller -->
<!-- plugins:js -->
<script src="assets/vendors/js/vendor.bundle.base.js"></script>
<!-- endinject -->
<!-- Plugin js for this page -->
<!-- End plugin js for this page -->
<!-- inject:js -->
<script src="assets/js/off-canvas.js"></script>
<script src="assets/js/hoverable-collapse.js"></script>
<script src="assets/js/misc.js"></script>
<script src="assets/js/settings.js"></script>
<script src="assets/js/todolist.js"></script>
<!-- endinject -->

<script>

    $(document).ready(function () {

        $(document).on('click', '#btnLogin', function () {
            login($('#txtUserId').val(),$('#txtPassword').val());
            return false;
        });

        $('#frmLogin').keypress(function (e) {
            if (e.which == 13) {
                login($('#txtUserId').val(),$('#txtPassword').val());
            }
        });

    });

    function login(user_id,password) {

        let action = 'login';

        let spinner = `<div class="spinner-border text-dark" role="status">
                          <span class="sr-only">Loading...</span>
                        </div>`;

        $("#btnLogin").attr('disabled', 'disabled').html(spinner);

        /*console.log('noman')
        return false;*/

        $.ajax({
            data: {
                'action': action,
                'user_id' : user_id,
                'password':password
            },
            type: 'POST',
            url: "include/ajax/action_login.php",
            success: function(result) {

                console.log(result);

                $("#btnLogin").text('Login');

                if (result == "000"){
                    window.location.href = "dashboard.php";
                }else if(result == "111"){
                    alert('User Name or Password not correct')
                    $("#btnLogin").attr('disabled', false).html('Login')
                }else{
                    alert('Something went worong try again later')
                    $("#btnLogin").attr('disabled', false).html('Login')
                }
            }
        });
    }

</script>

</body>
</html>

https://tutorialsclass.com/code/php-login-remember-cookies-script/
